...
void CheckDereference( UnaryOperationNode* Operation )
{
    ...
    else
    {
        ...
        // function pointers cannot be dereferenced except as a callee for
        // an indirect call, but that case is already handled by the parser
        else if( OperandPointerType->BaseType->Type() == DataTypes::Function )
          RaiseError( Operation->Location, "dereferencing a function pointer is only valid when calling it" );
    }        
}
